...
If you use a different deep-learning framework, specify that in the initial install, e.g., `[tensorflow]`, or if it is already installed you can omit the `[...]` part of the install command.
...
If the model has many parameter groups (i.e., lots of layers) and you get `too many files` errors, you may need to run `ulimit -n ${large_number}` before committing models, or else run `GIT_THETA_MAX_CONCURRENCY=${N} git add ${ckpt}.pt` to limit the number of parameters to process at once.
...
Currently, this code does *not* include tools to calculate statistics like those required by the Fisher Merging or MaTS.

Some scripts, like [majority\_sign.py](./git_theta_merge_cli/scripts/majority_sign.py) can be used to pre-calculate the majority sign across the trimmed task vectors for TIES merging. Currently, computations for dot products and model norms for SLERP/MLERP are (wastefully) redone during each merge. Similar scripts will (eventually) be added for per-computing these to avoid duplicated work.
...
Currently, LoRA parameters are merged independently, that is, the `A` matrix from each model is merged and the `B` matrix from each model is merged. However, most model statistics should probably be computed on `B @ A` anyway so it is probably better to create and commit new checkpoints with combined LoRA parameters and then use normal merging methods.

`--x:${param_name}=${param_value}` can be used to pass configuration to the merge method. For example, this is used for things like scaling with `--x:merge_lambda`, dropout probability with `--x:dropout_probability`, and conjugate gradient iterations, `--x:iterations`.
...
Currently, all merging operations are written using pytorch, however, they can still be used to merge models trained in other frameworks due to git-theta's plugin system.

For example, if your model is saved as a `safetensors` model, prefix each git-theta command (including the once used to check in the model) with `GIT_THETA_CHECKPOINT_TYPE=safetensors`.
...
> [!NOTE]
> This TIES implementation is different from the original implementation, for parameters whose value (summed across models) is zero use the majority sign from the *current parameter block* instead of from the *whole model*. The original implementation can be used via a two pass method. First run the `majority_sign` script over all the ties trimmed statistics (what will be used as the `--aux_data` in the merge command). This will output the majority sign for the *whole model*. This can then be used during merging with `--x:global_majority_sign=${sign}`.
...
git-theta-merge-cli \
  ...
  --x:norm ... \
  --x:norm ... \
  --output merges/mlerp/merged.pt \
  --merge mlerp \
```

Note: Both SLERP and MLERP can use pre-calculated values to avoid repeating work, SLERP can take the dot product value between the two normalized models and MLERP can take the parameter norms as well as the averaged norms. If these are provided, it can avoid re-computing them between different mreging experiments, otherwise they are re-computed on the fly.

When you are trying to merge 2 models, use `--merge slerp`. Use `--merge mlerp` when merging more.
...
You should also cite the original works for the merging method you use!
